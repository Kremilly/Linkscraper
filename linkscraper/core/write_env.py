...
class WriteEnv:
    ...
    def run(cls):
        imgur_key = Prompt.ask(f"Enter your Imgur Client ID (get "
                               f"[blue bold][link={Apis.IMGUR_API_KEY_URL.value}]here[/link][/blue bold])")
        
        virustotal_key = Prompt.ask(f"Enter your VirusTotal API key (get "
                                    f"[blue bold][link={Apis.VIRUSTOTAL_API_KEY_URL.value}]here[/link][/blue bold])")
        
        google_fonts_key = Prompt.ask(f"Enter your Google Fonts API Key (get "
                                      f"[blue bold][link={Apis.GOOGLE_FONTS_API_KEY_URL.value}]here[/link][/blue bold])")
